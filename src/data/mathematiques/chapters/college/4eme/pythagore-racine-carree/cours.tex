...
\section{23. Problème de diagonale}

Un écran rectangulaire mesure :
\[
48\ \text{cm}
\]
de largeur et :
...
Sa diagonale $d$ est l'hypoténuse d'un triangle rectangle.
...
À la calculatrice :
\[
d\approx55{,}07\ \text{cm}.
\]

Au millimètre près :
\[
\boxed{d\approx55{,}1\ \text{cm}}.
\]
...
\sqrt{3033}
\]
reste préférable tant qu'aucun arrondi n'est demandé.

\section{24. Combiner repérage et Pythagore}
...
L'écart horizontal vaut :
\[
4-(-2)=6.
\]

L'écart vertical vaut :
\[
9-1=8.
\]

Ces deux écarts sont les côtés de l'angle droit d'un triangle rectangle.
...
La géométrie dans le repère donne donc un contexte naturel d'utilisation du théorème.

\section{25. Contrôler une valeur approchée}

Supposons qu'un calcul donne :
\[
x=\sqrt{70}.
\]

Avant d'utiliser la calculatrice, on sait :
\[
8^2=64,
\]
et :
...
Si une calculatrice affiche :
\[
26{,}4,
\]
la valeur est manifestement impossible.

\begin{methode}
Pour contrôler une racine carrée :
\begin{enumerate}
\item chercher deux carrés parfaits voisins ;
\item encadrer le nombre sous la racine ;
\item en déduire un encadrement de la racine ;
\item comparer cet encadrement à l'affichage de la calculatrice.
\end{enumerate}
\end{methode}

Le contrôle d'ordre de grandeur fait partie intégrante du raisonnement mathématique.
