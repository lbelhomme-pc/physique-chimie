...
\section{18. Diviseur commun ou multiple commun ?}

Deux familles de problèmes utilisent les facteurs premiers mais ne posent pas la même question.

Pour constituer le plus grand nombre de groupes identiques à partir de plusieurs quantités, le nombre de groupes doit diviser chacune des quantités.

Pour trouver le premier instant où plusieurs phénomènes périodiques se reproduisent ensemble, la durée cherchée doit être un multiple de chaque période.
...
144
\]
objets bleus, un problème de lots maximaux demande un diviseur commun.

Deux machines fonctionnant toutes les :
\[
24\ \text{min}
\]
et :
...
36\ \text{min}
\]
conduisent au contraire à un multiple commun.
\end{exemple}

\section{19. Problème de pavage}
...
On veut utiliser les plus grands carreaux carrés possibles sans aucune découpe.

Le côté d'un carreau doit donc diviser les deux dimensions.
...
Le plus grand facteur commun vaut :
\[
2^2\times3\times5\times7=420.
\]

Le côté du carreau est :
\[
420\ \text{cm}.
\]
...
840\div420=2
\]
carreaux dans un sens et :
\[
1260\div420=3
\]
dans l'autre, soit :
\[
\boxed{6\text{ carreaux}}.
\]

\section{20. Montrer qu'une contrainte est impossible}

La factorisation permet aussi de démontrer qu'aucune solution n'existe.

On cherche un entier $n$ à la fois :
\begin{itemize}
\item multiple de $14$ ;
\item sans facteur premier commun avec $180$.
\end{itemize}
...
Tout multiple de $14$ est donc divisible par $2$.

Mais :
\[
180
\]
est également divisible par $2$.

Les deux conditions sont incompatibles.

On peut conclure :
\[
\boxed{\text{aucun entier }n\text{ ne convient}}.
\]

Ce type de raisonnement est important : en mathématiques, résoudre un problème peut aussi consister à démontrer qu'il n'a pas de solution.

\section{21. Erreurs fréquentes}
