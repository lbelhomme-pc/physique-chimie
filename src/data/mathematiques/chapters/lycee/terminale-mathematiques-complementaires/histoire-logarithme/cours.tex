...
\section{Repères historiques et enjeux du thème}
Historiquement, le logarithme a été introduit pour transformer les produits en sommes et simplifier les calculs numériques. Aujourd’hui, il permet aussi d’inverser la fonction exponentielle. Son usage exige une vigilance permanente sur le domaine de définition : toute expression placée dans un logarithme doit être strictement positive. Ce chapitre articule calcul algébrique, dérivation, limites et résolution d’équations.

\section{1. De l'histoire des tables à la fonction réciproque}
Les premières tables logarithmiques répondaient à un besoin de calcul : remplacer une multiplication difficile par une addition, puis revenir au résultat à l'aide d'une table inverse. Cette idée historique se retrouve exactement dans l'identité $\ln(ab)=\ln a+\ln b$.

La vision moderne est fonctionnelle : la fonction exponentielle est continue, strictement croissante sur $\mathbb{R}$ et prend toutes les valeurs strictement positives. Sa réciproque est la fonction logarithme népérien.

\section{2. Définition comme fonction réciproque}
...
\coursefigure{/images/mathematiques/terminale-mathematiques-complementaires/histoire-logarithme-1.svg}{Courbe de la fonction logarithme népérien.}{La courbe croissante passe par le point de coordonnées un zéro et met en évidence l’asymptote verticale d’équation x égale zéro.}
...
\coursefigure{/images/mathematiques/terminale-mathematiques-complementaires/histoire-logarithme-2.svg}{Comparaison qualitative entre logarithme et fonction affine.}{Le schéma montre que le logarithme croît de plus en plus lentement et reste sous sa tangente d’équation y égale x moins un.}
