...
export default function MegaQuizPlayer({ allQuestions }: { allQuestions: Question[] }) {
  ...
  const niveaux = [...new Set(allQuestions.map(q => q.niveau))].sort();
  const matieres = [...new Set(allQuestions.map(q => q.matiere))];
  ...
  const filtered = useMemo(() => {
    ...
    return f;
  }, [allQuestions, fNiveau, fMatiere, fChapter]);
  ...
  return (
    <div style={{background:V.bg,borderRadius:V.r,boxShadow:V.sh,padding:"1.5rem",border:`1px solid ${V.bd}`}}>
      ...
      <h3 style={{fontSize:"1.05rem",fontWeight:700,color:V.t,marginBottom:"1rem",lineHeight:1.4}}><KTex text={cur.question}/></h3>
      <div style={{display:"flex",flexDirection:"column",gap:"0.5rem"}}>
        {cur.choices.map((c,i)=>{
          ...
          return <button key={i} onClick={()=>{if(!rev)setSel(i);}} style={{padding:"0.7rem 1rem",borderRadius:V.rm,border:`2px solid ${bc}`,background:bg,color:co,fontWeight:600,fontSize:"0.9rem",textAlign:"left",cursor:rev?"default":"pointer",fontFamily:"inherit",transition:"all 0.15s"}}><KTex text={c}/></button>;
        })}
      </div>
      {rev&&cur.explanation&&<div style={{marginTop:"0.75rem",padding:"0.75rem 1rem",background:V.bgS,borderRadius:V.rm,fontSize:"0.85rem",color:V.tm}}>💡 <KTex text={cur.explanation}/></div>}
