...
for (const dir of chapterDirs) {
  ...
  for (const figureUrl of figurePaths) {
    if (!figureUrl.startsWith("/")) {
      errors.push(slug + ": figure non locale -> " + figureUrl);
      continue;
    }
    const figureFile = path.join(ROOT, "public", figureUrl.replace(/^\/+/, ""));
    if (!existsSync(figureFile)) {
      errors.push(slug + ": figure absente -> " + figureUrl);
      continue;
    }
  ...
  for (const exercise of exercises) {
    ...
    for (const item of exercise.curriculumItems ?? []) {
      curriculumCovered.add(item);
      if (!(meta.curriculumItems ?? []).includes(item)) errors.push(slug + ": curriculumItem hors chapitre -> " + exercise.id);
    }
...
if (chapterDirs.length !== EXPECTED_CHAPTERS) errors.push("C26: " + chapterDirs.length + " chapitres trouvés au lieu de " + EXPECTED_CHAPTERS);
